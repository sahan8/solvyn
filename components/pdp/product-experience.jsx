...
function BagDrawer({open,onOpenChange,bag}) { const reduced=useReducedMotion(); const subtotal=bag.items.reduce((sum,item)=>sum+item.price*item.quantity,0); return <Dialog.Root open={open} onOpenChange={onOpenChange}><AnimatePresence>{open&&<Dialog.Portal forceMount><Dialog.Overlay asChild><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[80] bg-black/55"/></Dialog.Overlay><Dialog.Content asChild><motion.aside initial={reduced?{opacity:0}:{x:'100%'}} animate={{x:0,opacity:1}} exit={reduced?{opacity:0}:{x:'100%'}} className="fixed inset-y-0 right-0 z-[81] flex w-full max-w-md flex-col bg-background p-5"><div className="flex items-center justify-between"><Dialog.Title className="font-display text-3xl font-semibold">YOUR BAG ({bag.count})</Dialog.Title><Dialog.Close className="grid size-11 place-items-center" aria-label="Close bag"><X/></Dialog.Close></div><p className="mt-2 flex items-center gap-2 text-sm text-green-800" aria-live="polite"><Check size={16}/> Added to bag</p><div className="mt-6 flex-1 overflow-y-auto divide-y divide-border">{bag.items.map((item)=>{const key=`${item.productId}:${item.variantId}:${item.size}`;return <div className="py-5" key={key}><div className="flex justify-between gap-3"><div><strong>{item.name}</strong><p className="mt-1 text-sm text-muted-foreground">{item.colorName} · US {item.size} · Qty {item.quantity}</p></div><span>${item.price*item.quantity}</span></div><button onClick={()=>bag.remove(key)} className="mt-3 text-xs underline">REMOVE</button></div>})}</div><div className="border-t border-black pt-5"><div className="flex justify-between font-semibold"><span>SUBTOTAL</span><span>${subtotal}</span></div><p className="mt-2 text-xs text-muted-foreground">Prices shown here are not authoritative. Future checkout will revalidate server-side.</p><Link href="/bag" className="mt-5 flex min-h-12 items-center justify-center border border-black font-semibold">VIEW BAG</Link><button disabled className="mt-2 min-h-12 w-full bg-black text-white opacity-45">CHECKOUT — COMING LATER</button></div></motion.aside></Dialog.Content></Dialog.Portal>}</AnimatePresence></Dialog.Root>; }